...
df = select(org.Hs.eg.db, keys = rn, columns = 'ENTREZID', keytype = 'UNIPROT')
df = na.omit(df)
table(duplicated(df$UNIPROT))
table(duplicated(df$ENTREZID))
# choose unique ids
i = match(rn, df$UNIPROT)
df = df[i,]
df = na.omit(df)
i = match(df$UNIPROT, rn)
rn = rn[i]
identical(rn, df$UNIPROT)
dim(df)
dim(na.omit(df))
...
oMsigGS.c2 = readList('~/Data/MetaData/c2.all.v6.2.entrez.gmt')
oMsigGS.c5 = readList('~/Data/MetaData/c5.all.v6.2.entrez.gmt')
oMsigGS.c7 = readList('~/Data/MetaData/c7.all.v6.2.entrez.gmt')
