...
dfContrast6.sub = ldfData[[6]][ldfData[[6]]$adj.P.Val < 0.01,]
dfContrast7.sub = ldfData[[7]][ldfData[[7]]$adj.P.Val < 0.01,]
...
lVenn = list(rownames(dfContrast1.sub), rownames(dfContrast2.sub), rownames(dfContrast3.sub), rownames(dfContrast4.sub),
             rownames(dfContrast5.sub), rownames(dfContrast6.sub), rownames(dfContrast7.sub))
names(ldfData)
#names(lVenn) = gsub('results//DEAnalysis(\\w+)VsControl.xls', '\\1', names(ldfData))
names(lVenn) = cvTitle
## drop the contrasts with small significant number of proteins
sapply(lVenn, length)
lVenn[c(4,5)] = NULL
# calculate overlaps
